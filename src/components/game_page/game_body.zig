...
pub fn detailBar(game: *const playday_api.models.game.Game, io: std.Io, allocator: std.mem.Allocator) !void {
    var detailsBox = dvui.box(@src(), .{}, .{ .expand = .horizontal });
...

    {
        if (game.grid) |grid| {
            ...
            const grid_height = grid_width / ratio;

            _ = dvui.image(
                @src(),
                .{
                    .shrink = .ratio,
                    .source = .{ .imageFile = .{ .bytes = grid } },
                },
                .{
                    .rect = .{
                        .x = detailsBox.wd.rect.w - grid_width,
                        .y = detailsBox.wd.rect.h - grid_height,
                        .w = grid_width,
                    },
                    .margin = .{ .w = 10 },
                    .corners = .all(15),
